...
namespace BugsFarm.Services.StatsService
{
    public class StatsCollection : Storage<Stat>, IStorageItem, IDisposable
    {
        ...
        public float GetVitalValue(string statId)
        {
            return Get<StatVital>(statId).CurrentValue;
        }
        ...
        public bool TryGet<T>(string statId, out T value) where T : Stat
        {
            value = default;
            if (!TryGet(statId, out var stat))
            {
                return false;
            }
            if (stat is T castable)
            {
                value = castable;
                return true;
            }
