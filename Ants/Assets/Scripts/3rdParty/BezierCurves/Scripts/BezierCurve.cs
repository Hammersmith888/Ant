...
public class BezierCurve : MonoBehaviour {
	...
	public bool _invertNormals = false;
	/// <summary>
	...
	public Vector3 GetPointAt(float t)
	{
		...
		{
			curvePercent = ApproximateLength(points[i-1], points[i], resolution) / length;
    ...
    private void OnDrawGizmosSelected()
    {
		...
		float step = 1f / (length * resolution);
		float t = 0f;
        while (t < 1f + step)
        {
			Gizmos.DrawRay(GetPointAt(t), GetNormalAt(t) /3);
			t += step;
		}
	...
	public Vector3 GetNormalAt(float t)
	{
		...
		if (t <= 0)
        {
            point0 = GetPointAt(step+step);
            point1 = GetPointAt(0.001f);
		}
        else if (t >= 1)
        {
            point0 = GetPointAt(0.999f);
            point1 = GetPointAt(0.999f - (step + step));
        }
        else
        {
            point0 = GetPointAt(t + step + step);
			point1 = GetPointAt(t);
        }

        Vector2 normal = Vector2.Perpendicular(point1 - point0).normalized;
        return _invertNormals ? normal * -1 : normal;
	...
	public static Vector3 GetPoint(BezierPoint p1, BezierPoint p2, float t)
	{
		if (p1 == null || p2 == null)
		{
			return p1? p1.position : p2? p2.position : Vector3.zero;
		}
